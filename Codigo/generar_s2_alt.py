...
pkts= []
capturados = 0
#aveces no filtra bien el filter arp
for capturados in range(total_packt):
    pktes=sniff(filter="arp",count= 1)

    if pktes[0].payload.name == 'ARP':
        pkts.append(pktes[0])
        #print 'se capturo arp'
    else:
        print 'se capturo erroneamente: ' + pktes[0].payload.name


total_packt = len(pkts)
